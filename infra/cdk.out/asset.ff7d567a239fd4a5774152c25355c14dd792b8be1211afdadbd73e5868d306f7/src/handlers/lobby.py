...
def answer_lobby_handler(event, context):
    ...
    # Determine if XP has already been awarded (first answer wins for both players)
    xp_already_awarded = bool(lobby.get("xp_awarded", False))
    xp_amount = 0
    item_reward = None
    host_dino = None
    guest_dino = None
    ...
    # Broadcast result to lobby channel so the other player sees it
    result_data = {
        "correct": is_correct,
